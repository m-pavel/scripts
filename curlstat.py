...
rrd_file = sys.argv[3] if len(sys.argv) >= 4 else "/tmp/speed.rrd"
rrd_totals = sys.argv[4] if len(sys.argv) >= 5 else None
...
#    print '{0} and {1} and {2}'.format(av_speed, react_speed, percent)
    rrdtool.update(rrd_file, 'N:%s' % (av_speed))
    if rrd_totals:
	rrdtool.update(rrd_totals, 'N:%s' % (av_speed))

def chunk_read(response, chunk_size=8192, report_hook=None):
    total_size = response.info().getheader('Content-Length').strip()
    total_size = int(total_size)
    bytes_so_far = len(response.read(chunk_size))
    starttime = time.time()
    chunktime = starttime
    while True:
...
#		    'DS:amount:COUNTER:10:U:U',
    ...
    ret = rrdtool.graph( output, "--start", str(rrdstart-5) , "--end", str(rrdend+5),
"--vertical-label=KB/s",
'--watermark='+time.strftime('%d-%m-%y %H:%M:%S %Z', time.localtime()),
"-w 800",
...
"GPRINT:m1_num:AVERAGE:Avg\: %6.0lf ",
"GPRINT:m1_num:MIN:Min\: %6.0lf ",
"GPRINT:m1_num:MAX:Max\: %6.0lf \\r")
